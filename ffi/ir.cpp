...
using ret = py::return_value_policy;
  ...
  // deepPipe自己的dialect相关的函数，与Python绑定部分
void init_ffi_ir_deepPipe(py::module_ &&m) {
  m.def("load_dialects", [](MLIRContext &context) {
    mlir::DialectRegistry registry;
    // clang-format off
    registry.insert<mlir::deepPipe::DeepPipeDialect,
...
void init_ffi_ir(py::module_ &&m) {
  ...
  init_ffi_ir_deepPipe(m.def_submodule("deepPipe"));
}
